...
def remove_link(processed_link, input_file='input.txt'):
    ...
    with open(input_file, 'w') as file:
        for file_link in file_links:
            if file_link.strip() != processed_link:
                file.write(file_link)

# Step 1: URL Input and Scraping
url = log.input("Enter Fitgirl Game Link (leave empty to resume from input.txt): ")
...
if url.strip():
    ...
    if not scraped_links:
        log.error("No Matching URLs Found", "Exiting")
        raise SystemExit(1)
    else:
        log.success("Links found and saved to input.txt", len(scraped_links))
        with open('input.txt', 'w') as file:
            file.write("\n".join(scraped_links) + "\n")

# Step 2: Read Links and Process Downloads
try:
    with open('input.txt', 'r') as file:
        links = [line.strip() for line in file if line.strip()]
except FileNotFoundError:
    log.error("input.txt not found", "Please provide a valid URL first.")
    raise SystemExit(1)

if not links:
    log.warning("input.txt is empty", "No links to download. Please provide a new URL.")
    raise SystemExit(1)
